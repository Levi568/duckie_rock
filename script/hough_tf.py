...
class hough_class:
    ...
    def hough_image(self, image_msg):
        ...
        white, yellow = self.image_filter(image_np)

        try:
            self.pub_white.publish(self.bridge.cv2_to_imgmsg(white, "bgr8"))
            self.pub_yellow.publish(self.bridge.cv2_to_imgmsg(yellow, "bgr8"))
